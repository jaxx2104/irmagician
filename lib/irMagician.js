        ...
        this.port.write("T\r\n", (error, bytesWritten) => {
            ...
            this.port.on("data", data => {
                ...
                if (msg[0]) {
                    ...
                    callback ? callback(celsiusTemp) : console.log(celsiusTemp)
                    resolve()
                } else {
                    resolve()
                }
...
exports.play = (path, port, callback) => {
    co(function* () {
        var validPort = yield autoSelectDevicePort()
        port = port || validPort
        yield open(port)
        if (file) { yield write(path) }
        yield play()
        yield close()
    }).catch(error => {
        console.log(`💀  Error: ${error}`)
...
exports.temp = (path, port, callback) => {
    co(function* () {
        var validPort = yield autoSelectDevicePort()
        port = port || validPort
        yield open(port)
        yield temp(path, callback)
        yield close()
    }).catch(error => {
        console.log(`💀  Error: ${error}`)
        close()
